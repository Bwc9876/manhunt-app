 ...
 */
async startLobby(joinCode: string | null, settings: GameSettings) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("start_lobby", { joinCode, settings }) };
} catch (e) {
...
export type LobbyState = { profiles: Partial<{ [key in string]: PlayerProfile }>; join_code: string; 
/**
 * True represents seeker, false hider
 */
teams: Partial<{ [key in string]: boolean }>; self_seeker: boolean; settings: GameSettings }
/**
